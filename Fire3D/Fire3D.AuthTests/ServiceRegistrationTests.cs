...
{
    [Fact]
    public void Aws_configuration_rejects_only_one_static_credential()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => new ServiceCollection().AddApplication(config));
    }

    [Fact]
    public async Task Storage_uses_environment_bucket_when_config_bucket_is_blank()
    {
        var config = new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?>
        {
            ["AWS:Region"] = "ap-northeast-1",
        ...
        var previousBucket = Environment.GetEnvironmentVariable("S3_BUCKET_NAME");
        ...
        Environment.SetEnvironmentVariable("S3_BUCKET_NAME", "fire3d-environment-test-bucket");
        ...
        try
        {
            ...
            Assert.Contains("fire3d-environment-test-bucket", url, StringComparison.Ordinal);
        }
        finally
        {
            Environment.SetEnvironmentVariable("S3_BUCKET_NAME", previousBucket);
