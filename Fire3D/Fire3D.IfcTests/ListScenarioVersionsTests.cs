...
public sealed class ListScenarioVersionsTests
{
    [Theory]
    ...
    public async Task Invalid_or_unauthorized_requests_do_not_read_store(UserRole role, int expectedStatus)
    {
        ...
        var result = await new ListScenarioVersionsQueryHandler(RevisionAccessTests.Accounts(actor), store)
    ...
    public async Task List_uses_database_organization_scope(UserRole role)
    {
        ...
        var page = new PageResponse<ScenarioVersionSummaryResponse>([], 0, 1, 20);
        var store = StubProxy.For<IScenarioReadStore>((method, args) =>
        {
            Assert.Equal(nameof(IScenarioReadStore.ListScenarioVersionsAsync), method);
            Assert.Equal(scenarioId, args[0]);
            Assert.Equal(role == UserRole.PlatformAdmin ? null : actor.OrganizationId, args[1]);
            return Task.FromResult<PageResponse<ScenarioVersionSummaryResponse>?>(page);
        });
    ...
    public async Task Missing_or_foreign_scenario_is_not_found()
    {
        ...
        var store = StubProxy.For<IScenarioReadStore>((_, _) => Task.FromResult<PageResponse<ScenarioVersionSummaryResponse>?>(null));
        var result = await new ListScenarioVersionsQueryHandler(RevisionAccessTests.Accounts(actor), store)
